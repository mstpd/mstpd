...
get_protocol_version() {
    local bridge="$1"
    mstpctl_get showbridge "${bridge}" force-protocol-version
}
...
test_T16_01_migrate_force_stp() {
    ...
    if [[ "${version}" == "stp" ]]; then
        ...
        test_fail "Force STP mode expected 'stp' but got '${version}'"
        return 1
    fi
}
...
test_T16_02_migrate_force_rstp() {
    ...
    if [[ "${version}" == "rstp" ]]; then
        ...
        test_fail "Force RSTP mode expected 'rstp' but got '${version}'"
        return 1
    fi
}
...
test_T16_03_migrate_force_mstp() {
    ...
    if [[ "${version}" == "mstp" ]]; then
        ...
        test_fail "Force MSTP mode expected 'mstp' but got '${version}'"
        return 1
    fi
}
...
test_T16_04_migrate_stp_to_rstp() {
    ...
    # Switch br1 to RSTP
    mstpctl setforcevers "br1" rstp || true
...
test_T16_05_migrate_rstp_to_stp() {
    ...
    # Poll for br0 to detect STP peer (protocol migration takes time,
    # especially with STP's 30s convergence cycle)
    local send_rstp="yes"
    local rcvd_stp="no"
    local start_time
    start_time=$(date +%s)
    while (( $(date +%s) - start_time < 45 )); do
        send_rstp=$(mstpctl_get showportdetail "br0" "br0-br1" send-rstp)
        rcvd_stp=$(mstpctl_get showportdetail "br0" "br0-br1" received-stp)
    ...
    if [[ "${rcvd_stp}" == "yes" ]] || [[ "${send_rstp}" == "no" ]]; then
        ...
        test_fail "RSTP did not downgrade to STP: send_rstp='${send_rstp}', rcvd_stp='${rcvd_stp}'"
        return 1
    fi
}
...
test_T16_06_migrate_mstp_to_rstp() {
    ...
    if bridge_is_root "br0" || bridge_is_root "br1"; then
        ...
        test_fail "MSTP/RSTP interoperation failed: neither bridge became root"
        cleanup_topology_b "br0" "br1"
        return 1
    fi
}
...
test_T16_07_migrate_mcheck() {
    ...
    mstpctl portmcheck "br0" "br0-br1" || true
