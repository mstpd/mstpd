...
test_T3_03_port_link_up() {
    ...
    if [[ "${output}" == *"veth0"* ]]; then
        ...
        if [[ -n "${state}" ]] && [[ "${state}" != "0" ]]; then
            # Verify kernel and mstpd agree on state
            if port_verify_state "br0" "veth0"; then
                log_debug "Kernel and mstpd state consistent"
            else
                log_debug "Kernel and mstpd state differ (may need more convergence time)"
            fi
