...
test_T4_01_stp_root_election() {
    ...
    # Set distinct priorities so we can predict the root (lower = better)
    bridge_enable_stp "br0" "stp"
    bridge_enable_stp "br1" "stp"
    mstpctl settreeprio "br0" 0 4 || true
    mstpctl settreeprio "br1" 0 8 || true

    # Wait for convergence
    wait_for_convergence 30

    local br0_root br1_root
    br0_root=$(bridge_is_root "br0" && echo "yes" || echo "no")
    br1_root=$(bridge_is_root "br1" && echo "yes" || echo "no")

    log_debug "br0 root: ${br0_root}, br1 root: ${br1_root}"

    cleanup_topology_b "br0" "br1"

    # br0 (priority 4) should be root, br1 (priority 8) should not
    if [[ "${br0_root}" == "yes" ]] && [[ "${br1_root}" == "no" ]]; then
        test_pass "Root bridge elected correctly (br0 with lower priority)"
        return 0
    else
        test_fail "Expected br0 as root (prio 4 < 8), got br0=${br0_root} br1=${br1_root}"
        return 1
    fi
}
...
test_T4_03_stp_two_bridges() {
    ...
    if [[ "${br0_port_role}" == "Designated" && "${br1_port_role}" == "Root" ]] ||
       [[ "${br0_port_role}" == "Root" && "${br1_port_role}" == "Designated" ]]; then
...
test_T4_04_stp_triangle() {
    ...
    # In a triangle with 3 bridges, there is exactly 1 loop.
    # STP must block exactly 1 port to break it.
    if [[ ${blocked} -eq 1 ]]; then
        test_pass "Triangle topology: exactly 1 port blocked"
        return 0
    elif [[ ${blocked} -gt 1 ]]; then
        # More than 1 blocked is over-blocking but still loop-free
        test_pass "Triangle topology: ${blocked} ports blocked (over-blocking)"
        return 0
    else
