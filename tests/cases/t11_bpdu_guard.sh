...
test_T11_01_bpduguard_enable() {
    ...
    # Enable BPDU guard
    mstpctl setbpduguard "br0" "br0-p1" yes

    local bpduguard
    bpduguard=$(mstpctl_get showportdetail "br0" "br0-p1" bpdu-guard-port)
    ...
    if [[ "${bpduguard}" == "yes" ]]; then
        ...
        test_fail "BPDU guard not enabled (got: ${bpduguard:-empty})"
        return 1
    fi
}
...
test_T11_02_bpduguard_disable() {
    ...
    mstpctl setbpduguard "br0" "br0-p1" yes
    mstpctl setbpduguard "br0" "br0-p1" no
    ...
    if [[ "${bpduguard}" == "no" ]]; then
        ...
        test_fail "BPDU guard not disabled (got: ${bpduguard:-empty})"
        return 1
    fi
}
...
test_T11_03_bpduguard_trigger() {
    ...
    mstpctl setbpduguard "br0" "br0-br1" yes

    sleep 3

    # Check if BPDU guard error occurred
    local bpduguard_error
    bpduguard_error=$(mstpctl_get showportdetail "br0" "br0-br1" bpdu-guard-error)
    ...
    if [[ "${bpduguard_error}" == "yes" ]]; then
        ...
        test_fail "BPDU guard error not triggered (got: ${bpduguard_error:-empty})"
        return 1
    fi
}
...
test_T11_04_bpduguard_edge() {
    ...
    mstpctl setportadminedge "br0" "br0-p1" yes
    mstpctl setbpduguard "br0" "br0-p1" yes
    ...
    if [[ "${admin_edge}" == "yes" ]] && [[ "${bpduguard}" == "yes" ]]; then
        ...
        test_fail "BPDU guard + edge not both enabled (edge=${admin_edge:-empty}, guard=${bpduguard:-empty})"
        return 1
    fi
}
...
test_T11_05_bpduguard_recovery() {
    ...
    mstpctl setbpduguard "br0" "br0-br1" no
