...
test_T5_02_rstp_proposal_agreement() {
    ...
    if [[ ${converged} -eq 1 ]] && [[ ${duration} -lt 5 ]]; then
        ...
        test_fail "RSTP converged but too slowly (${duration}s, expected < 5s)"
        return 1
    else
...
test_T5_07_rstp_sync() {
    ...
    # Wait for each leaf port to reach forwarding (state 3)
    for port in br1-br0 br2-br0 br3-br0; do
        wait_for_port_state "${port}" 3 15 || true
    done

    # Verify all ports are forwarding
    local all_synced=1
    for port in br1-br0 br2-br0 br3-br0; do
        local bridge="${port%%-*}"
        local state
