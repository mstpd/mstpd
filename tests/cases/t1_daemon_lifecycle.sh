...
# T1.08: Duplicate instance detection via PID file lock
test_T1_08_daemon_duplicate() {
    ...
    # Try to start second instance in foreground — should fail on socket bind
    "${MSTPD_BIN}" -d -v 2 > "${log_file}" 2>&1 &
    local second_pid=$!
    disown "${second_pid}" 2>/dev/null || true
    ...
    if [[ "${count}" -gt 1 ]]; then
        ...
        test_fail "Second mstpd instance was allowed to start (${count} running)"
        return 1
    fi

    # Verify first instance still running with same PID
...
# T1.09: PID file cleanup on daemon exit
test_T1_09_daemon_pid_cleanup() {
    test_start "T1.09" "daemon_pid_cleanup"

    mstpd_stop 2>/dev/null || true

    # Start daemon (daemonizes, creates PID file)
    "${MSTPD_BIN}" -v 2 > "${TEST_LOG_DIR}/mstpd.log" 2>&1
    sleep 0.5

    local pid
    pid=$(pgrep -x mstpd 2>/dev/null | head -1)
    if [[ -z "${pid}" ]]; then
        test_fail "Could not start mstpd"
        return 1
    fi

    # Verify PID file exists (path matches --localstatedir from configure)
    if [[ ! -f "${MSTPD_PID_FILE}" ]]; then
        log_debug "PID file not found at ${MSTPD_PID_FILE}, skipping"
        kill -TERM "${pid}" 2>/dev/null
        sleep 0.5
        test_skip "PID file not created"
        return 0
    fi

    log_debug "PID file exists before stop: $(cat "${MSTPD_PID_FILE}")"
    ...
    if [[ -f "${MSTPD_PID_FILE}" ]]; then
        test_fail "PID file not cleaned up after daemon exit"
        rm -f "${MSTPD_PID_FILE}" 2>/dev/null
        return 1
    else
        test_pass "PID file cleaned up on daemon exit"
        return 0
    fi
}
