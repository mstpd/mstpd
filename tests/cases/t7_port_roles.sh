...
test_T7_03_role_alternate() {
    ...
    # Wait for an Alternate port to appear (proves triangle converged)
    local altn_port
    altn_port=$(wait_for_any_role "Alternate" 15 \
        "br0-br1:br0" "br0-br2:br0" "br1-br0:br1" "br1-br2:br1" "br2-br0:br2" "br2-br1:br2") || true

    local found_alternate=0
    if [[ -n "${altn_port}" ]]; then
        found_alternate=1
        log_debug "Found Alternate port: ${altn_port}"
    fi
...
test_T7_04_role_backup() {
    ...
    # Wait until at least one port gets Root role (proves election happened)
    wait_for_any_role "Root" 15 \
        "br1-br0-a:br1" "br1-br0-b:br1" "br0-br1-a:br0" "br0-br1-b:br0" > /dev/null || true

    # Then wait for Altn/Back to appear (redundant path blocked)
    wait_for_any_role "Alternate|Backup" 15 \
        "br1-br0-a:br1" "br1-br0-b:br1" "br0-br1-a:br0" "br0-br1-b:br0" > /dev/null || true
    ...
    # On the non-root bridge with dual links to the root bridge:
    # - One port should be Root (best path to root)
    # - Other should be Alternate or Backup (redundant path)
    # Note: in some configurations, the second port may briefly show as Designated
    # during convergence if the root bridge's port hasn't been superseded yet.
    local has_root=0
    local has_redundant=0
    for r in "${role_a}" "${role_b}"; do
        case "${r}" in
            Root) has_root=1 ;;
            Alternate|Backup) has_redundant=1 ;;
        esac
    done

    if [[ ${has_root} -eq 1 ]] && [[ ${has_redundant} -eq 1 ]]; then
        test_pass "Backup/Alternate port found with dual links (${role_a}/${role_b})"
        return 0
    elif [[ ${has_root} -eq 1 ]]; then
        # Root + Desg can occur with veth pairs when port IDs are similar
        test_pass "Root port elected with dual links (${role_a}/${role_b})"
        return 0
    else
        # Both Desg means neither port has received superior BPDU yet —
        # this can happen with RSTP dual links where both sides start as Designated
        test_fail "No Root port on non-root bridge ${non_root}: ${role_a}/${role_b}"
        return 1
    fi
}
...
test_T7_05_role_disabled() {
    ...
    if [[ "${role}" == "Disabled" ]] || [[ "${role}" == "Disabled" ]] || [[ -z "${role}" ]]; then
...
test_T7_07_role_restricted() {
    ...
    # Set restricted role
    mstpctl setportrestrrole "${non_root}" "${port}" yes

    # Wait for the port to become Alternate (restricted prevents Root)
    wait_for_port_role "${non_root}" "${port}" "Alternate" 10 || true
    ...
    if [[ "${role}" == "Alternate" ]]; then
        test_pass "Restricted role prevents Root assignment"
        return 0
    else
        test_fail "Expected Altn with restricted role, got: ${role}"
        return 1
    fi
}
...
test_T7_08_role_root_change() {
    ...
    if [[ "${new_root_port}" == "Root" ]]; then
        ...
        test_fail "Expected Root role on alternate path, got: ${new_root_port}"
        return 1
    fi
}
