...
test_T19_01_err_invalid_bridge() {
    ...
    if [[ ${ret} -ne 0 ]] || [[ "${output}" == *"error"* ]] || [[ "${output}" == *"No such"* ]] || [[ -z "${output}" ]]; then
        ...
        test_fail "Invalid bridge name not rejected (rc=${ret}, output: ${output})"
        return 1
    fi
}
...
test_T19_03_err_invalid_param() {
    ...
    # Try invalid parameter values — daemon should not crash and should not apply them
    mstpctl settreeprio "br0" 0 999 || true
    mstpctl sethello "br0" 999 || true
    mstpctl setmaxage "br0" 999 || true
    mstpctl setfdelay "br0" 999 || true
    ...
    # Verify hello time was rejected (IEEE mandates hello=2, invalid value ignored)
    local hello
    hello=$(mstpctl_get showbridge "br0" hello-time)
    log_debug "Hello time after invalid set: ${hello}"

    cleanup_topology_a "br0" 1

    if [[ "${hello}" == "2" ]]; then
        test_pass "Invalid parameters rejected (hello stayed at ${hello})"
        return 0
    else
        test_fail "Hello time should be 2 after invalid set (got: ${hello:-empty})"
        return 1
    fi
}
...
test_T19_08_err_permission() {
    ...
    if [[ -n "${output}" ]]; then
        ...
        test_fail "showbridge returned empty output"
        return 1
    fi
}
