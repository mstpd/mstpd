...
test_T17_01_stats_rx_bpdu() {
    ...
    # Get RX BPDU counter
    local rx_bpdu
    rx_bpdu=$(mstpctl_get showportdetail "br0" "br0-br1" num-rx-bpdu)
    ...
    if [[ -n "${rx_bpdu}" ]] && [[ "${rx_bpdu}" =~ ^[0-9]+$ ]] && [[ "${rx_bpdu}" -gt 0 ]]; then
        ...
        test_fail "RX BPDU counter not positive after 5s (got: ${rx_bpdu:-empty})"
        return 1
    fi
}
...
test_T17_02_stats_tx_bpdu() {
    ...
    # Get TX BPDU counter
    local tx_bpdu
    tx_bpdu=$(mstpctl_get showportdetail "br0" "br0-br1" num-tx-bpdu)
    ...
    if [[ -n "${tx_bpdu}" ]] && [[ "${tx_bpdu}" =~ ^[0-9]+$ ]] && [[ "${tx_bpdu}" -gt 0 ]]; then
        ...
        test_fail "TX BPDU counter not positive after 5s (got: ${tx_bpdu:-empty})"
        return 1
    fi
}
...
test_T17_05_stats_fwd_trans() {
    ...
    local fwd_trans
    fwd_trans=$(mstpctl_get showportdetail "br0" "br0-br1" num-transition-fwd)
...
test_T17_06_stats_blk_trans() {
    ...
    local blk_trans
    blk_trans=$(mstpctl_get showportdetail "br0" "br0-br1" num-transition-blk)
...
test_T17_08_stats_uptime() {
    ...
    # Get time since TC
    local time_since_tc
    time_since_tc=$(mstpctl_get showbridge "br0" time-since-topology-change)
