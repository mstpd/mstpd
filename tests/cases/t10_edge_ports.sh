...
test_T10_01_edge_admin_yes() {
    ...
    # Set admin edge
    mstpctl setportadminedge "br0" "br0-p1" yes || true

    sleep 1

    # Check if port is edge
    local admin_edge
    admin_edge=$(mstpctl_get showportdetail "br0" "br0-p1" admin-edge-port)
    ...
    if [[ "${admin_edge}" == "yes" ]]; then
        ...
        test_fail "Admin edge expected 'yes' but got '${admin_edge}'"
        return 1
    fi
}
...
test_T10_02_edge_admin_no() {
    ...
    mstpctl setportadminedge "br0" "br0-p1" no || true
    ...
    if [[ "${admin_edge}" == "no" ]]; then
        ...
        test_fail "Admin edge expected 'no' but got '${admin_edge}'"
        return 1
    fi
}
...
test_T10_03_edge_auto_yes() {
    ...
    # Set auto edge
    mstpctl setportautoedge "br0" "br0-p1" yes || true

    sleep 1

    local auto_edge
    auto_edge=$(mstpctl_get showportdetail "br0" "br0-p1" auto-edge-port)
    ...
    if [[ "${auto_edge}" == "yes" ]]; then
        ...
        test_fail "Auto edge expected 'yes' but got '${auto_edge}'"
        return 1
    fi
}
...
test_T10_04_edge_auto_no() {
    ...
    # Set auto edge to no
    mstpctl setportautoedge "br0" "br0-p1" no || true
    ...
    if [[ "${auto_edge}" == "no" ]]; then
        ...
        test_fail "Auto edge expected 'no' but got '${auto_edge}'"
        return 1
    fi
}
...
test_T10_05_edge_operEdge() {
    ...
    local oper_edge
    oper_edge=$(mstpctl_get showportdetail "br0" "br0-p1" oper-edge-port)
    ...
    if [[ "${oper_edge}" == "yes" ]]; then
        ...
        test_fail "Operational edge expected 'yes' but got '${oper_edge}'"
        return 1
    fi
}
...
test_T10_06_edge_bpdu_received() {
    ...
    mstpctl setportadminedge "br0" "br0-br1" yes || true

    # Poll until oper edge becomes "no" (BPDU received clears edge status)
    local oper_edge="yes"
    local start_time
    start_time=$(date +%s)
    while (( $(date +%s) - start_time < 15 )); do
        oper_edge=$(mstpctl_get showportdetail "br0" "br0-br1" oper-edge-port)
    ...
    if [[ "${oper_edge}" == "no" ]]; then
        ...
        test_fail "Oper edge expected 'no' after BPDU but got '${oper_edge}'"
        return 1
    fi
}
...
test_T10_07_edge_revert() {
    ...
    # Poll until auto-edge detects no BPDUs and sets oper edge to yes
    local oper_edge="no"
    local start_time
    start_time=$(date +%s)
    while (( $(date +%s) - start_time < 15 )); do
    ...
    if [[ "${oper_edge}" == "yes" ]]; then
        ...
        test_fail "Oper edge expected 'yes' (no peer) but got '${oper_edge}'"
        return 1
    fi
}
...
test_T10_08_edge_fast_forward() {
    ...
    if [[ ${forwarding} -eq 1 ]]; then
        ...
        test_fail "Edge port did not reach forwarding state within 5s"
        return 1
    fi
}
